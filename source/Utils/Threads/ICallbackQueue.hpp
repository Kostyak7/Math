...
template<typename F>
auto fem::make_safe_callback(F callback, std::weak_ptr<const void> lifetime) {
...
template<typename F>
auto fem::make_safe_callback(F callback, std::weak_ptr<const void> lifetime, std::weak_ptr<ICallbackQueue> callback_queue) {
...
template <typename F>
void fem::call_in_worker(F callback, std::weak_ptr<ICallbackQueue> callback_queue, std::optional<std::weak_ptr<const void>> lifetime) {
...
std::future<R> fem::SyncCaller<BaseClass>::sync(R(DerivedClass::* method)(MArgs...), Args&&... args) {
...
template <typename BaseClass>
template <typename F>
std::future<std::invoke_result_t<F>> fem::SyncCaller<BaseClass>::add_sync(F&& callback, std::optional<std::weak_ptr<const void>> lifetime) {
...
template <typename BaseClass>
template<typename DerivedClass, typename R, typename... MArgs, typename... Args>
std::future<R> fem::AsyncCaller<BaseClass>::async(R(DerivedClass::* method)(MArgs...), Args&&... args) {
    static_assert(std::is_base_of_v<BaseClass, DerivedClass>, "Mismatch deriving");
...
template <typename BaseClass>
template <typename F>
std::future<std::invoke_result_t<F>> fem::AsyncCaller<BaseClass>::add_async(F&& callback, std::optional<std::weak_ptr<const void>> lifetime) {
    static_assert(std::is_base_of_v<ICallbackQueue, BaseClass>, "Caller does not belong to ICallbackQueue");
...
template <typename BaseClass>
template<typename DerivedClass, typename R, typename... MArgs, typename... Args>
std::future<R> fem::TypedCaller<BaseClass>::typed_call(ExecutionType type, R(DerivedClass::* method)(MArgs...), Args&&... args) {
...
template <typename BaseClass>
template <typename F>
std::future<std::invoke_result_t<F>> fem::TypedCaller<BaseClass>::add_command(ExecutionType type, F&& callback, std::optional<std::weak_ptr<const void>> lifetime) {
    if (type == ExecutionType::SYNCHRONE) {
