template<class T>
mthread::BlockingQueue<T>::BlockingQueue(size_t max_size, OnOverflow on_overflow, OnBecomeNormal on_become_normal)
    : m_max_size(max_size)
...
template<class T>
void mthread::BlockingQueue<T>::pop(T& item) {
...
template<class T>
bool mthread::BlockingQueue<T>::try_pop(T& item) {
    return pop_impl(item, [this](auto& /*lock*/) { return !m_queue.empty(); });
}

template<class T>
template <typename Duration>
bool mthread::BlockingQueue<T>::try_pop(T& item, const Duration duration) {
...
template<class T>
void mthread::BlockingQueue<T>::push(T item) {
...
template<class T>
bool mthread::BlockingQueue<T>::try_push(T item) {
...
template<class T>
size_t mthread::BlockingQueue<T>::size() const {
    std::scoped_lock lock(m_mutex);
    return m_queue.size();
}

template<class T>
void mthread::BlockingQueue<T>::clear() {
    std::queue<T> q;
    std::scoped_lock lock(m_mutex);
    m_queue.swap(q);
}
...
template<class T>
void mthread::BlockingQueue<T>::set_max_size(size_t size) {
    std::scoped_lock lock(m_mutex);
    m_max_size = size;
}
